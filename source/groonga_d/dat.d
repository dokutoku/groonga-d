...
/+
#define GRN_DAT_EACH(ctx, dat, id, key, key_size, block) groonga_d.dat.grn_dat_cursor *_sc = groonga_d.dat.grn_dat_cursor_open(ctx, dat, null, 0, null, 0, 0, -1, 0); if (_sc) { groonga_d.groonga.grn_id id; uint *_ks = (key_size); if (_ks) { while ((id = groonga_d.dat.grn_dat_cursor_next(ctx, _sc))) { int _ks_raw = groonga_d.dat.grn_dat_cursor_get_key(ctx, _sc, cast(const (void)**)(key)); *(_ks) = cast(uint)(_ks_raw); block } } else { while ((id = groonga_d.dat.grn_dat_cursor_next(ctx, _sc))) { groonga_d.dat.grn_dat_cursor_get_key(ctx, _sc, cast(const (void)**)(key)); block } } groonga_d.dat.grn_dat_cursor_close(ctx, _sc); }
+/
