...
/+
#define GRN_ARRAY_EACH(ctx, array, head, tail, id, value, block) groonga_d.array.grn_array_cursor *_sc = groonga_d.array.grn_array_cursor_open(ctx, array, head, tail, 0, -1, 0); if (_sc) { groonga_d.groonga.grn_id id; while ((id = groonga_d.array.grn_array_cursor_next(ctx, _sc))) { groonga_d.array.grn_array_cursor_get_value(ctx, _sc, cast(void**)(value)); block } groonga_d.array.grn_array_cursor_close(ctx, _sc); }

#define GRN_ARRAY_EACH_BEGIN(ctx, array, cursor, head, tail, id) do { groonga_d.array.grn_array_cursor *cursor; cursor = groonga_d.array.grn_array_cursor_open((ctx), (array), (head), (tail), 0, -1, 0); if (cursor) { groonga_d.groonga.grn_id id; while ((id = groonga_d.array.grn_array_cursor_next(ctx, cursor))) {

#define GRN_ARRAY_EACH_END(ctx, cursor) } groonga_d.array.grn_array_cursor_close(ctx, cursor); } } while (0)
+/
