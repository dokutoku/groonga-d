...
#define GRN_HASH_EACH_BEGIN(ctx, hash, cursor, id) do { grn_hash_cursor *cursor; cursor = grn_hash_cursor_open((ctx), (hash), null, 0, null, 0, 0, -1, GRN_CURSOR_BY_ID); if (cursor) { groonga_d.groonga.grn_id id; while ((id = grn_hash_cursor_next((ctx), cursor)) != groonga_d.groonga.GRN_ID_NIL) {
