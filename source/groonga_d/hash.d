...
/+
#define GRN_HASH_EACH(ctx, hash, id, key, key_size, value, block) grn_hash_cursor *_sc = grn_hash_cursor_open(ctx, hash, null, 0, null, 0, 0, -1, 0); if (_sc) { uint id; while ((id = grn_hash_cursor_next(ctx, _sc))) { grn_hash_cursor_get_key_value(ctx, _sc, (void **)(key), (key_size), (void **)(value)); block } grn_hash_cursor_close(ctx, _sc); }

#define GRN_HASH_EACH_BEGIN(ctx, hash, cursor, id) do { grn_hash_cursor *cursor; cursor = grn_hash_cursor_open((ctx), (hash), null, 0, null, 0, 0, -1, GRN_CURSOR_BY_ID); if (cursor) { uint id; while ((id = grn_hash_cursor_next((ctx), cursor)) != groonga_d.groonga.GRN_ID_NIL) {

#define GRN_HASH_EACH_END(ctx, cursor) } grn_hash_cursor_close((ctx), cursor); } } while(0)
+/
